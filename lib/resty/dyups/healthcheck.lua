...
local function set_peer_down(ctx, is_backup, id, value)
    ...
    local dict = ctx.dict
    local server_list = cjson.decode(dict:get(u))
    server_list[id + 1]['down'] = value
    dict:set("status_changed", true)
    return dict:set(u, cjson.encode(server_list))
end

local function get_primary_peers(dict, u)
    local server_list = dict:get(u)
    if server_list == nil then
        return nil
    end
    server_list = cjson.decode(server_list)
    return server_list
end
...
function _M.spawn_checker(opts)
    ...
    local ppeers = get_primary_peers(dict, u)
    if not ppeers then
        ppeers = config.server_list
    end
    if not ppeers then
        return nil, "failed to get server_list"
    end
    ...
    local ok, err = dict:set(u, cjson.encode(ppeers))
    if not ok then
        errlog("failed to set server list: ", err)
    end
